...
SBMcmc::SBMcmc(SBModel* m, TreeParameter* t, SBMatrix* cm, Settings& s, bool dBO) : 
    model(m), codonMatrix(cm), tree(t), generalUpdates(3), stationaryUpdates(5), treeUpdates(0), optim(7, s.bayesOptFrequency), disableBayesOpt(dBO), truncations(s.truncation) { 
    ...
    */
    ...
    if(bayesOptIter > 0){
        Msg::error("Bayesian optimization not implemented for stick breaking model!");
    }
    ...
    proportionChoice = s.proportionsWeight + stationaryChoice;
...
double SBMcmc::GibbsIteration(double currentLnPosterior){
    RandomVariable& rng = RandomVariable::randomVariableInstance();

    double randomMove = rng.uniformRv() * proportionChoice;
    ...
    for(int n = 1; n <= numBurnIn; n++){
        if(n % printFreq == 0){
            ...
            std::cout << "Accept Rates Since Last Tuning Iteration:" << 
                         ...
                         "\tProportion Rate=" << (double)codonMatrix->proportionAcceptCount/(double)codonMatrix->proportionCount << std::endl;
        }
        if(n % tuneFreq == 0){
            model->tuneMoves();
        }
