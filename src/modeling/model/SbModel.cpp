...
double SBModel::lnPrior(){
    return tree->lnPrior() + rateMatrix->lnPrior();
}

void SBModel::regenerateLikelihood(){
    TreeObject* activeT = tree->getTree();
...
void SBModel::tuneMoves(){
    tree->tune();
    rateMatrix->tune();
}

std::string SBModel::tabularHeader(){
    std::string returnString = "Iteration\tPosterior\tLikelihood\tPrior\tK\tR";
    for(int i = 0; i < rateMatrix->getOmegas().size(); i++){
        returnString += "\tOmega[" + std::to_string(i) + "]";
        returnString += "\tProportion[" + std::to_string(i) + "]";
    }
    for(int i = 0; i < 61; i++){
...
std::string SBModel::tabularOut(int i){
    ...
    for(int i = 0; i < omegas.size(); i++){
        returnString += "\t" + std::to_string(omegas[i]) + "\t" + std::to_string(proportions[i]);
    }
...
std::string SBModel::ancestralHeader(){
    std::string returnString = "Iteration";
...
std::tuple<std::string, std::string> SBModel::reconstructionOut(int i){
    RandomVariable& rng = RandomVariable::randomVariableInstance();
    ...
    for(int d = 0; d < numJointDraws; d++){
        ...
        taskMap.insert(std::make_pair(root->getIndex(), phyloTaskflow.emplace([this, root, &rng, reconstructedStates, reconstructeddNdS, dNdSVec](){
        ...
        for(Node* n : preOrderSeq){
            if(n != activeT->getRoot()){
                taskMap.insert(std::make_pair(n->getIndex(), phyloTaskflow.emplace([this, n, &rng, reconstructedStates, reconstructeddNdS, dNdSVec](){
                    int nIndex = n->getIndex();
...
std::string SBModel::branchHeader(){
...
std::string SBModel::branchOut(int i){
    std::string returnString = std::to_string(i) + "\t" + std::to_string(lnPrior() + currentLikelihood);
    TreeObject* treeObj = tree->getTree();
    std::vector<Node*> poSeq = treeObj->getPostOrderSeq();
    std::vector<Node*> nodes;
